<template>
    <div class="folderContainer" draggable="true" :class="folderHoverClass" @dragstart="dragStart" @dragover="fileHover"
        ...
        <div class="folderWrapper" @click.right.prevent="rightClick">
            <div class="folder" v-if="props.isDir">
                <div class="front"></div>
                <div class="center"></div>
                <div class="back"></div>
            </div>
            <el-icon :size="80" v-else>
                <VideoPlay />
            </el-icon>
    ...
    <context-menu v-model:show="show" :options="optionsComponent">
        <context-menu-item label="delete" @click="onMenuClick(FileMenuOperate.Delete)" />
    </context-menu>
</template>
...
function onMenuClick(operate: FileMenuOperate) {
    switch (operate) {
...
const props = withDefaults(defineProps<File>(), { name: "unknown", isDir: false, lastModified: "2021/07/17 10:49" })
